...
public class Player extends Entity{
    ...
    public Player(GamePanel gp, KeyHandler keyH) {
        ...
        // Subtract a half tile length from both screenX and screenY so the player is in the middle
    ...
    public void update() {
        ...
        // Makes it so images only switch if a key is pressed
        if (keyH.upPressed || keyH.downPressed || keyH.leftPressed || keyH.rightPressed) {
            ...
            spriteCounter ++;
            if (spriteCounter > 12) { // Makes the image change every 12 frames
                if (spriteNum == 1) {  // Changes image from 1 to 2
                    spriteNum = 2;

                } else if (spriteNum == 2) { // Changes image from 2 to 1
                    spriteNum = 1;
                }
    ...
    public void draw(Graphics2D g2) {
        ...
        switch(direction) { // Switches between two images so it looks like the player is moving
            case "up":
                if(spriteNum == 1) {
                    image = up1;
                }
